...
export function initLocale(locale, changeLang, updateLocale) {
  ...
  if (pathLang === 'en') {
    ...
    // 其他语言路径
    finalLang = `${pathLang}-${pathLang.toUpperCase()}`
  } else {
    // 根路径,按优先级选择语言
    finalLang = queryLang || storedLang || (browserLang?.startsWith('zh') ? 'zh-CN' : 'en-US')
  }

  // 4. 应用语言设置
  if (finalLang) {
    const match = finalLang.match(/[a-zA-Z]{2}(?:-[a-zA-Z]{2})?/)
    if (match) {
...
export const redirectUserLang = (lang, pageId) => {
  ...
  // 没有开启多语言
  if (BLOG.NOTION_PAGE_ID.indexOf(',') < 0) return
  ...
  if (pathLang) {
    // URL路径优先
    userLang = pathLang === 'en' ? 'en-US' : `${pathLang}-${pathLang.toUpperCase()}`
  } else if (queryLang) {
    // 查询参数次之
    userLang = queryLang
  } else if (storedLang) {
