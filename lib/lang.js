...
export const redirectUserLang = (lang, pageId) => {
  ...
  const pathLang = currentPath === '/' ? 'zh' : currentPath.split('/')[1]
  ...
  for (let index = 0; index < siteIds.length; index++) {
    ...
    // 比较基础语言代码
    if (prefix && userLangBase === prefix.toLowerCase()) {
      // 如果是默认语言(中文),且当前路径不是根路径,则重定向到根路径
      if (userLangBase === 'zh' && window.location.pathname !== '/') {
        window.location.href = '/'
      } 
      // 其他语言则重定向到带前缀的路径
      else if (userLangBase !== 'zh' && window.location.pathname.indexOf(prefix) < 0) {
